...
function update(frameTime, lag) {
	...
	for (let i = BurstText.active.length - 1; i >= 0; i--) {
		const text = BurstText.active[i];
		text.life -= timeStep;
		text.opacity = text.life / 2000; // 根据剩余生命计算透明度

		if (text.life <= 0) {
			BurstText.active.splice(i, 1);
			BurstText.returnInstance(text);
		} else {
...
class Shell {
	constructor(options) {
		...
		this.isMainShell = !options._isSubShell; // 添加标记来区分主烟花和子效果
	...
	burst(x, y) {
		...
		// 只在主烟花爆炸时显示文字
		if (this.isMainShell) {
			const randomTextGroup = BURST_TEXTS[Math.floor(Math.random() * BURST_TEXTS.length)];
			BurstText.add(x, y, randomTextGroup);
		}
	}
}
...
const BurstText = {
    ...
    add(x, y, textGroup) {
        ...
        instance.x = x;
        instance.y = y;
        instance.text = textGroup[0];
        instance.opacity = 1;
        instance.life = 2500; // 增加一点显示时间，让文字更容易看清
        this.active.push(instance);
        return instance;
    },
